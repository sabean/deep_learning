...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        layer_activation = np.maximum(0, (X @ W1) + b1)
        scores = np.dot(layer_activation, W2) + b2
        ...
        loss = loss / y.shape[0] + (0.5 * reg * np.sum(W1**2)) + (0.5 * reg * np.sum(W2**2))
        ...
        dW2 = np.dot(layer_activation.T, softmax)
        db2 = np.sum(softmax, axis=0)
        
        hidden_gradient = np.dot(softmax, W2.T)
        hidden_gradient[layer_activation <= 0] = 0
    ...
    def predict(self, X):
        ...
        layer1_activation = np.maximum(0, (X @ W1) + b1)
        layer2_activation = (layer1_activation @ W2) + b2
        
        y_pred = np.argmax(layer2_activation, axis=1)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    regularization_strengths = [0.08, 0.1, 0.12]
    hidden_sizes = [100, 200, 300]
    
    
    for learn_units in learning_rates:
        for reg_units in regularization_strengths:
            for h in hidden_sizes:
                net = TwoLayerNet(input_size, h, num_classes)
                net.train(X_train, y_train,X_val, y_val, learning_rate=learn_units, reg=reg_units,
                ...
                results[(learn_units, reg_units, h)] = (train_accuracy, val_accuracy)
                
    for (lr, reg, h) in sorted(results):
        train_accuracy, val_accuracy = results[(lr, reg, h)]
        print('learning rate: %e  regularization:  %e  hidden size:  %d training accuracy: %f  val accuracy: %f' % (
